...
### ⚠️ 生产实测：限流有效，但**不是 100%**（诚实记录）

2026-09-21 对生产环境实测（每请求换一个错误 token）：

| 观测       | 结果                                                                              |
| ---------- | --------------------------------------------------------------------------------- |
| 连续 20 次 | `429 403 429 429 403 403 403 429 429 403 429 429 403 429 429 429 403 403 429 429` |
| 连续 60 次 | **58 次 429 / 2 次 403 → 拦截率 97%**                                             |

**结论：限流确实生效，但不是 100% 拦截。**

根因是 serverless 的实例隔离 —— **Edge isolate 之间不共享内存**。
每个请求可能落到不同 isolate，各自持有一份独立的失败计数。
所以「5 次失败即封禁」在全局视角下会被稀释：攻击者看到的实际阈值高于 5。

> ⚠️ **踩坑记录**：最初用「连续 12 次」采样时全部返回 403，我一度误判为
> 「限流完全没生效」。加大样本后才发现是**样本量不足**导致的误判 ——
> 小样本在概率性防御上极易得出相反结论。验证此类机制必须用足够大的样本，
> 并统计分布而非看单次结果。

**这个局限可接受吗**：对个人站点可接受。97% 的拦截率足以拖死自动化穷举
（且真正的防线是 token 本身 + 失败方向的 fail-closed）。
**但不要误以为它是硬性闸门** —— 若需要精确定量，必须换成
边缘 KV / Redis 等共享计数器（Upstash 等，需评估成本）。
...
## 五之二、生产环境验证（2026-09-21 实际执行）

| 项                                                | 结果                                                                            |
| ------------------------------------------------- | ------------------------------------------------------------------------------- |
| `ADMIN_TOKEN` 写入 Vercel（production + preview） | ✅ type=sensitive                                                               |
| `ADMIN_TOKEN` 写入 GitHub secrets                 | ✅（同时修掉 P1-3 缺口）                                                        |
| 生产重新部署                                      | ✅ 见下方「env 变更与部署时序」                                                 |
| 新 token 请求 `POST /api/news`                    | **200 `{"success":true}`**                                                      |
| 错误 token 请求                                   | **403**（鉴权正常）                                                             |
| middleware 是否运行                               | ✅ `GET /api/admin/revalidate` 返回 **403**（而非 405）证明 middleware 拦截生效 |
| 限流拦截率                                        | **97%**（58/60）                                                                |

### ⚠️ env 变更与部署时序（真实踩坑）

**Vercel 的环境变量变更只对「之后开始的构建」生效。** 本次实测踩到：

- 10:08:10 merge 触发生产部署**开始构建**
- 10:08:42 更新 `ADMIN_TOKEN`
- 结果：该部署**没带上新 token**，生产仍拒绝新 token（实测 403）
- 修复：显式重新部署（`POST /v13/deployments` 带 `deploymentId`），10:11:55 开始构建 → 新 token 生效

**教训**：改完 Vercel env **必须重新部署**，且要确认新部署的 `created` 时间**晚于**
env 变更时间。只看「部署成功」会误判 —— 部署是成功的，只是用的旧配置。

> 另注：本站在 Cloudflare 后面（`server: cloudflare`），但 Vercel 仍会正确设置
> 客户端 IP 头（实测伪造 `x-forwarded-for` / `x-real-ip` / `x-vercel-forwarded-for`
> 都不影响限流计数），与 [Vercel 文档](https://vercel.com/docs/headers/request-headers)
> 「覆盖以阻止 IP 伪造」一致。
