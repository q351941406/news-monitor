...
**两类监控互补，覆盖不同层面的故障**（「站点活着但数据断流」必须靠后者）：

| 故障类型                     | 告警来源                                                                   |
| ---------------------------- | -------------------------------------------------------------------------- |
| 站点 / Serverless 函数不可用 | 本监控（`/api/health`，5 分钟，liveness）                                  |
| **管道级故障（数据断流）**   | **`freshness-check.yml`**（每天 3 次，UTC 6/14/22）                        |
| 数据库不可用                 | GitHub Actions 抓取任务失败通知（业务自身告警）；需要时手动 `?deep=1` 排查 |

### 为什么必须有 `freshness-check`（不是重复建设）

**真实事故**：Product Hunt 断流 47 天、X 断流 57 天，全程 workflow 绿色、
**零告警**。原因是：

- UptimeRobot 只探活 `/api/health` —— 站点**活得好好的**
- 抓取 workflow 也「成功」（空结果被伪装成正常）

于是「站点正常 + 任务成功 + 零数据」这个组合**没有任何监控能发现**。

`freshness-check.yml` 把「数据新鲜度」变成**主动检查**：任一源超过预期周期
未更新 → 非 0 退出 → workflow 红 → GitHub 通知。它**不经过 Port 或任何
被动拉取接口**，因此不会在故障时一起失效。

| 项       | `uptime-monitoring`（本文档） | `freshness-check.yml`         |
| -------- | ----------------------------- | ----------------------------- |
| 检查对象 | 进程/站点存活                 | 数据是否在更新                |
| 频率     | 5 分钟                        | 每天 3 次                     |
| 是否查库 | ❌（省 Neon 额度）            | ✅ 必须查（要读最新抓取时间） |
| 触发方式 | 外部服务轮询                  | cron 主动推                   |
| 覆盖故障 | 站点挂了                      | 站点活着但断流                |
